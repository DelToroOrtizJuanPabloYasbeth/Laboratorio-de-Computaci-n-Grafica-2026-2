...
Window::Window()
{
	...
	luzPrendida = true;
	colorCiclo = 0;
	reiniciarAnimacion = false; // Tecla R
	accionT = false;            // Inicialización tecla T
...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	// --- LÓGICA DE REINICIO (Tecla R) ---
	if (key == GLFW_KEY_R && action == GLFW_PRESS)
	{
		theWindow->reiniciarAnimacion = true;
	}

	// --- LÓGICA DE ACCIÓN (Tecla T) ---
	if (key == GLFW_KEY_T && action == GLFW_PRESS)
	{
		theWindow->accionT = true;
	}
	...
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		if (key == GLFW_KEY_K) {
			...
			theWindow->direccionHeli = 2;
		}
		if (key == GLFW_KEY_L) {
			theWindow->muevex -= 0.5f;
			theWindow->direccionHeli = 1;
		}
		...
		// --- MOVIMIENTO PEZ / COFRE (TECLAS F y G) --- 
		...
		// --- CICLO DE COLORES (Tecla Q) ---
		if (key == GLFW_KEY_Q && action == GLFW_PRESS)
		{
			theWindow->colorCiclo++;
			if (theWindow->colorCiclo > 5)
			{
