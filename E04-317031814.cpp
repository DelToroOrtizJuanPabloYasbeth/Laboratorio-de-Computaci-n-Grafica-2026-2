...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// articulacion1 hasta articulación5 sólo son puntos de rotación o articulación
		...
		// =============================================
		// LLANTAS (Relación con la Base/Pirámide)
		// =============================================

		// --- LLANTA 1 ---
		model = modelaux;
		// X: 2.2f para que sobresalga del ancho (4.0/2 = 2.0)
		// Y: -0.5f para que esté por debajo del centro de la base
		// Z: 1.5f para que esté en el frente
		model = glm::translate(model, glm::vec3(1.5f, -0.8f, 1.8f));

		// Rotaciones para orientar el cilindro correctamente
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));

		// Rotación independiente de rodado
		...
		// --- LLANTA 2 ---
		model = modelaux;
		// Cambiamos 1.5f a -1.5f para moverla atras
		model = glm::translate(model, glm::vec3(-1.5f, -0.8f, 1.8f));

		// Mantienes exactamente las mismas rotaciones para que herede la orientación
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		...
		// --- LLANTA 3 ---
		model = modelaux;
		// Cambiamos 1.8f a -1.8f para moverla al otro lado
		model = glm::translate(model, glm::vec3(1.5f, -0.8f, -1.8f));
		...
		// -- - LLANTA 4 -- -
		model = modelaux;
		// Cambiamos 1.8f y 1.5 a -1.8f y -1.5 para moverla al otro lado atras
		model = glm::translate(model, glm::vec3(-1.5f, -0.8f, -1.8f));
