...
int main()
{
	...
	pointLights[0] = PointLight(1.0f, 0.0f, 0.0f,
		...
		-6.0f, 1.5f, 1.5f,
		0.3f, 0.2f, 0.1f);
	pointLightCount++; //incremenrar contador de luces
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// DECLARACIÓN ÚNICA DE VARIABLES DE MATRIZ
		glm::mat4 model(1.0);
		...
		// CUERPO-Padre (Movimiento con K/L)
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		//FARO AZUL 
		glm::vec3 posFaro = glm::vec3(modelaux[3]);
		...
		spotLights[2].SetFlash(posFinalFaro, dirFaro);

		//COFRE-Hijo (Rotación con )
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 0.72f, 1.1f));
		model = glm::rotate(model, mainWindow.getcofre() * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Cofre_M.RenderModel();

		//LLANTAS-Hijas (Rotacion con H/J) 
		// Delantera Izquierda
		model = modelaux;
		model = glm::translate(model, glm::vec3(-0.5f, 0.3f, 1.4f));
